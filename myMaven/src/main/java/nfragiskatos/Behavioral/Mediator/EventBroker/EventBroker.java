package nfragiskatos.Behavioral.Mediator.EventBroker;

import io.reactivex.Observable;
import io.reactivex.Observer;

import java.util.ArrayList;
import java.util.List;

/**
 * Here the EventBroker is the mediator.
 *
 * Using reactive extensions instead of rolling own observer class in order to create a mediator. Here we supply values
 * and publish events to subscribers.
 */
public class EventBroker extends Observable<Integer> {

    private List<Observer<? super Integer>> observers = new ArrayList<>();

    @Override
    protected void subscribeActual(Observer<? super Integer> observer) {
            observers.add(observer);
    }

    public void publish(int n) {
        for (Observer<? super Integer> observer : observers) {
            observer.onNext(n);
        }
    }
}
